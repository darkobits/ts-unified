...
export async function resolveBin(pkgName: string, binName?: string) {
  ...
  if (!pkgPath) {
    throw new Error(
      `[resolveBin] Unable to resolve path to package "${pkgName}".`
    );
  }
  ...
  if (!pkgData) {
    throw new Error(
      '[resolveBin] Unable to find a package.json for the local project.'
    );
  }

  if (!pkgData.packageJson.bin) {
    throw new Error(
      `[resolveBin] Package "${pkgName}" does not declare any binaries.`
    );
  }
  ...
  const relativeBinPath = binName
    ? pkgData.packageJson.bin[binName]
    : pkgData.packageJson.bin[pkgName];

  if (!relativeBinPath) {
    throw new Error(
      `[resolveBin] Package "${pkgName}" does not have binary "${
        binName ?? pkgName
      }".`
    );
  }
...
export async function requireBin(pkgName: string, binName?: string) {
  ...
  log.verbose(
    'bin',
    `Using ${log.chalk.bold(`${binName ?? pkgName}`)} version ${log.chalk.green(
      binInfo.version
    )}.`
  );
...
export function findTsConfig() {
  ...
  try {
    ...
    if (err.code === 'ENOENT') {
      log.warn(
        log.prefix('ts-unified'),
        [
          "Attempted to automatically set ESLint's parserOptions.project to",
          `${log.chalk.green(
            tsConfigPath
          )}, but the file does not exist. You may need to set`,
