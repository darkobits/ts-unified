...
import { EXTENSIONS_WITH_DOT, SRC_DIR, OUT_DIR } from 'etc/constants';
...
export default (userArgument) => {
  ...
  scripts.lint = {
    ...
    script: `${prefixBin(
      'eslint'
    )} src --ext .ts,.tsx,.js,.jsx --format=node_modules/eslint-codeframe-formatter`,
    fix: {
      description: 'Lint the project and automatically fix all fixable errors.',
      script: `${prefixBin(
        'eslint'
      )} src --ext .ts,.tsx,.js,.jsx --format=node_modules/eslint-codeframe-formatter --fix`
    }
  };

  scripts.format = {
    description: 'Format the project',
    script: `${prefixBin('prettier')} "src/**/*.{js,ts,tsx,jsx}" --check`,
    fix: {
      description:
        'Format the project and automatically fix all fixable errors.',
      script: `${prefixBin('prettier')} src --ext .ts,.tsx,.js,.jsx --write`
    }
  };
  ...
  const ttsc = `${prefixBin('tspc')} --pretty`;
  ...
  const postBuild = `${prefixBin(
    'del'
  )} "${OUT_DIR}/**/*.spec.*" "${OUT_DIR}/**/*.test.*"`;
  ...
  scripts.bump = {
    ...
    beta: {
      description:
        'Generates a change log and tagged commit for a beta release.',
      script: npsUtils.series(
        ...[
          // If there is a user-defined script named 'prebump', run it.
          userScripts?.scripts?.prebump ? 'nps prebump' : undefined,
    ...
    first: {
      description:
        "Generates a changelog and tagged commit for a project's first release.",
      script: npsUtils.series(
        ...[
          // If there is a user-defined script named 'prebump', run it.
          userScripts?.scripts?.prebump ? 'nps prebump' : undefined,
  ...
  scripts.prepare = {
    description:
      'Runs after "npm install" to ensure the package compiles correctly.',
    script: npsUtils.series(
      scripts.build.default.script,
      `${scripts.test.default.script} --passWithNoTests`
    )
  };
